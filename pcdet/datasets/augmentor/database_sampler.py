...
class DataBaseSamplerMM(object):
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict):
        ...
        for idx, info in enumerate(total_valid_sampled_dict):
            ...
            if self.sampling_method == 'LiDAR-aware':
                ...
                obj_points = self.random_drop_out(obj_points, rand_noise=self.occlusion_noise,
                                                  offset=self.occlusion_offset)
    ...
    def __call__(self, data_dict):
        ...
        for class_name, sample_group in self.sample_groups.items():
            ...
            if int(sample_group['sample_num']) > 0:
                ...
                sampled_boxes = copy.deepcopy(sampled_boxes1)
                sampled_boxes[:, 0] += np.random.random() * (
                            self.max_sampling_dis - self.min_sampling_dis) + self.min_sampling_dis
