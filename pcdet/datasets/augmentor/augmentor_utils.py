...
def transform_backward(pred_boxes, transformation):
    enable_flip = transformation[1].item()
    for box in pred_boxes:
        # world scale
        box['pred_boxes'][:, 0:6] /= transformation[2]
        # flip
        if enable_flip == 1.0:
            box['pred_boxes'][:, 1] = -(box['pred_boxes'][:, 1])
            box['pred_boxes'][:, 6] = -(box['pred_boxes'][:, 6])
        # rotate
        box['pred_boxes'][:, 0:3] = common_utils.rotate_points_along_z(box['pred_boxes'][np.newaxis, :, 0:3],
                                                                       -transformation[:][np.newaxis, 0])[0]
        box['pred_boxes'][:, 6] += -transformation[:][np.newaxis, 0]
    return pred_boxes
