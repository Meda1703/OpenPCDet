...
class TTAugmentor(object):
    def __init__(self, augmentor_configs=None):
        ...
        self.rot_num = len(self.aug_config_list[0]['WORLD_ROT_ANGLE'])
    ...
    def world_rotation(self, data_dict=None, config=None):
        ...
        if 'boxes' in data_dict:
            ...
            if self.backward_flag:
                boxes_lidar[:, 0:3] = common_utils.rotate_points_along_z(boxes_lidar[np.newaxis, :, 0:3], -rot_factor)[
                    0]
    ...
    def input_transform(self, data_dict, trans_boxes=True):
        """
        Args:
            data_dict:
                points: (N, 3 + C_in)
