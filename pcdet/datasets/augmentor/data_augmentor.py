    ...
    def random_world_rotation(self, data_dict=None, config=None):
        ...
        if 'roi_boxes' in data_dict.keys():
            ...
            roi_boxes, _, _ = augmentor_utils.global_rotation(
                data_dict['roi_boxes'].reshape(-1, dim), np.zeros([1, 3]), rot_range=rot_range, return_rot=True,
    ...
    def random_world_scaling(self, data_dict=None, config=None):
        ...
        if 'roi_boxes' in data_dict.keys():
            gt_boxes, roi_boxes, points, noise_scale = augmentor_utils.global_scaling_with_roi_boxes(
                data_dict['gt_boxes'], data_dict['roi_boxes'], data_dict['points'], config['WORLD_SCALE_RANGE'],
                return_scale=True
            )
